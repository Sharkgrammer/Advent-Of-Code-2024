...
def manual_valid(manual, was_invalid=False):
        ...
        if p in rules:
            for rule in rules.get(p):
                if rule in page_list:
                    rule_index = page_list.index(rule)

                    manual[page], manual[rule_index] = manual[rule_index], manual[page]

                    return manual_valid(manual, True)
